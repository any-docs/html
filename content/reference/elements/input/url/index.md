...
The CSS gives visual clues to show the user if the content is {{cssxref(":valid")}} or {{cssxref(":invalid")}} by adding an appropriate {{cssxref("content")}} property and includes [alternative text](/css/reference/properties/content#alternative_text_string_counter) for users with assistive technologies.
