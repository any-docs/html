...
- `wrap`
  - Indicates how the control should wrap the value for form submission. Possible values are:
    ...
    - `off` <Badge type="info" text="Non-standard. Check cross-browser support before using." />: Like `soft` but changes appearance to `white-space: pre` so line segments exceeding `cols` are not wrapped and the `<textarea>` becomes horizontally scrollable.
