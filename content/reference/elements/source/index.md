...
- `media`
  - Specifies the [media query](/css/guides/Media_queries) for the resource's intended media.
...
A [media query](/css/guides/Media_queries/Using) allows the browser to select an image to display with the `height` and `width` attributes based on the [viewport](/en-US/docs/Glossary/Viewport) size.
