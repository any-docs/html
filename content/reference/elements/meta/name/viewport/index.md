  ...
  - The `content` attribute must be defined, and its value sets various viewport-related behaviors.
    ...
    - `interactive-widget`
      - Specifies the effect that interactive UI widgets, such as virtual keyboards, have on a page's viewport.
        ...
        When the {{Glossary("viewport")}} gets resized, the initial [containing block](/css/guides/Display/Containing_block) also gets resized, thereby affecting the computed size of [viewport units](/css/reference/values/length#relative_length_units_based_on_viewport).
    ...
    - `viewport-fit`
      - Defines the viewable portions of the web page.
        ...
        - `cover`: The viewport is scaled to fill the device display.
          It's highly recommended to use the [safe area inset](/css/reference/values/env) variables to ensure that important content doesn't end up outside the display.
