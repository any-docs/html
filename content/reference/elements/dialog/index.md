...
This example demonstrates a modal dialog with a [gradient](/css/reference/values/gradient) backdrop. The `.showModal()` method opens the modal dialog when the "Show the dialog" button is activated. The dialog can be closed by pressing the <kbd>Esc</kbd> key or via the `close()` method when the "Close" button within the dialog is activated.
...
> [!NOTE]
> When animating using [CSS transitions](/css/guides/Transitions), [`transition-behavior: allow-discrete`](/css/reference/properties/transition-behavior) needs to be set to enable the above behavior. This behavior is available by default when animating with [CSS animations](/css/guides/Animations); an equivalent step is not required.
