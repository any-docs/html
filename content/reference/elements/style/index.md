...
In the same manner as `<link>` elements, `<style>` elements can include `media` attributes that contain [media queries](/css/guides/Media_queries), allowing you to selectively apply internal stylesheets to your document depending on media features such as viewport width.
...
- `media`
  - This attribute defines which media the style should be applied to. Its value is a [media query](/css/guides/Media_queries/Using), which defaults to `all` if the attribute is missing.
- `nonce`
...
In this example we've included two `<style>` elements — notice how the conflicting declarations in the later `<style>` element override those in the earlier one, if they have equal [specificity](/css/guides/Cascade/Specificity).
