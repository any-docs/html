...
By default, the minimum is 0 and the maximum is 100. If that's not what you want, you can easily specify different bounds by changing the values of the [`min`](/en-US/docs/Web/HTML/Reference/Elements/input#min) and/or [`max`](/en-US/docs/Web/HTML/Reference/Elements/input#max) attributes. These can be any floating-point value.
...
If you want to accept any value regardless of how many decimal places it extends to, you can specify a value of `any` for the [`step`](/en-US/docs/Web/HTML/Reference/Elements/input#step) attribute:
...
To add tick marks to a range control, include the `list` attribute, giving it the `id` of a {{HTMLElement("datalist")}} element which defines a series of tick marks on the control. Each point is represented using an {{HTMLElement("option")}} element with its [`value`](/en-US/docs/Web/HTML/Reference/Elements/option#value) set to the range's value at which a mark should be drawn.
...
See [Creating vertical form controls](/css/guides/Writing_modes/Vertical_controls) for examples.
